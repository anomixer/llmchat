...
2. **數據安全**: 用戶數據和對話記錄存儲在本地文件系統，不會上傳到雲端
3. **數據備份**: 管理員應定期備份 `data/users.json` 文件，包含所有用戶帳戶和對話記錄
4. **會話管理**: 登入狀態在服務器重啟時會重置，但會在 7 天內保持有效
5. **管理員權限**: 管理員可以管理所有用戶帳戶，請妥善保管管理員密碼
6. **數據恢復**: 如需恢復用戶數據，從備份文件複製到 `data/` 目錄並重啟服務器
7. **硬體要求**: 大型模型需要足夠的 RAM（建議 8GB 以上）
8. **首次使用**: 首次啟動模型可能需要下載，請耐心等待
9. **性能**: 模型大小會影響響應速度
