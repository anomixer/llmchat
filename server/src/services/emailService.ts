...
class EmailService {
    ...
    // 發送驗證email
    async sendVerificationEmail(email: string, verificationUrl: string, userName = '', language = 'zh-TW') {
        if (!this.transporter) {
    ...
    // 獲取本地化郵件主題
    getLocalizedSubject(language: string) {
    ...
    // 驗證email模板
    getVerificationEmailTemplate(verificationUrl: string, userName = '', language = 'zh-TW') {
        // 語言映射
