...
import { getSystemPrompt, normalizeLanguage, type SupportedLanguage } from './prompts.js'
...
// 認證中間件
const authenticateToken = (req: AuthedRequest, res: Response, next: NextFunction) => {
...
function getVerificationPageHTML(isSuccess: boolean, errorMessage: string, language: SupportedLanguage, frontendUrl: string) {
...
// 用戶登出
app.post('/api/auth/logout', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
        const authHeader = req.headers['authorization']
...
// 驗證會話
app.get('/api/auth/verify', authenticateToken, (req: AuthedRequest, res: Response) => {
    res.json({ user: req.user })
})

// 多語言錯誤消息
const verificationErrorMessages: Record<SupportedLanguage, { expired: string; invalid: string }> = {
    'zh-TW': {
...
// Email 驗證
app.get('/api/auth/verify-email/:token', (req: Request, res: Response) => {
    try {
...
app.get('/api/models', async (req: Request, res: Response) => {
    try {
        const apiUrl = (req.query.apiUrl as string) || 'http://localhost:11434'
...
app.post('/api/chat', authenticateToken, async (req: AuthedRequest, res: Response) => {
    try {
        ...
        const chatSettings = buildChatSettings(req.body)
...
// 停止流式請求端點
app.post('/api/chat/stop', (req: Request, res: Response) => {
    try {
...
app.post('/api/chat/stream', authenticateToken, async (req: AuthedRequest, res: Response) => {
...
// 獲取用戶的對話列表
app.get('/api/conversations', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
...
// 保存用戶的對話
app.post('/api/conversations', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
...
// 獲取用戶設定
app.get('/api/user/settings', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
...
// 更新用戶設定
app.post('/api/user/settings', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
...
// 更改密碼
app.post('/api/user/change-password', authenticateToken, async (req: AuthedRequest, res: Response) => {
    try {
...
// 管理員 API - 獲取所有用戶
app.get('/api/admin/users', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
...
// 管理員 API - 更新用戶角色
app.put('/api/admin/users/:userId/role', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
        if (!userService.isAdmin(req.user!.userId)) {
            return res.status(403).json({ error: '需要管理員權限' })
        }
...
// 管理員 API - 刪除用戶
app.delete('/api/admin/users/:userId', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
        if (!userService.isAdmin(req.user!.userId)) {
            return res.status(403).json({ error: '需要管理員權限' })
        }
...
// 管理員 API - 切換用戶啟用狀態
app.put('/api/admin/users/:userId/toggle-enable', authenticateToken, (req: AuthedRequest, res: Response) => {
    try {
        if (!userService.isAdmin(req.user!.userId)) {
            return res.status(403).json({ error: '需要管理員權限' })
        }
